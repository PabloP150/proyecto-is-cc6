-- Insert sample analytics data using existing task IDs
-- Get some existing task IDs first
DECLARE @task1 UNIQUEIDENTIFIER = (SELECT TOP 1 tid FROM dbo.Tasks ORDER BY tid);
DECLARE @task2 UNIQUEIDENTIFIER = (SELECT tid FROM dbo.Tasks WHERE tid != @task1 ORDER BY tid OFFSET 1 ROWS FETCH NEXT 1 ROWS ONLY);
DECLARE @task3 UNIQUEIDENTIFIER = (SELECT tid FROM dbo.Tasks WHERE tid NOT IN (@task1, @task2) ORDER BY tid OFFSET 2 ROWS FETCH NEXT 1 ROWS ONLY);
...
('ta111111-1111-1111-1111-111111111111', @task1, '77777777-7777-7777-7777-777777777777', '00000000-0000-0000-0000-000000000789', 'general', '2024-12-03 09:00:00', '2024-12-03 16:00:00', 'completed', 7.0),
('ta222222-2222-2222-2222-222222222222', @task2, '11111111-1111-1111-1111-111111111111', '00000000-0000-0000-0000-000000000456', 'backend', '2024-12-04 10:00:00', NULL, 'pending', NULL),
('ta333333-3333-3333-3333-333333333333', @task3, '33333333-3333-3333-3333-333333333333', '00000000-0000-0000-0000-000000000456', 'frontend', '2024-12-04 14:00:00', NULL, 'pending', NULL);
